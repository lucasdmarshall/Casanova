...
error body.
    ...
    python -m casanova_hub.mcp_server
"""
...
server = MCPServer(
    ...
    version="0.1.0",
    instructions=(
        "The Casanova tool hub — one endpoint for web search/fetch, code "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Casanova hub MCP server")
    parser.add_argument(
        "--transport",
        default="stdio",
    ...
    try:
        ...
        print(f"[casanova-hub] backends up: {health['services_up']}/{health['services_total']}")
