...
**One HTTP + MCP endpoint for the whole [Casanova](https://github.com/lucasdmarshall/Casanova) toolset.**
...
Plus a **federating MCP server** (`python -m casanova_hub.mcp_server`): agents
...
```bash
pip install -e ".[mcp]"
uvicorn casanova_hub.service:app --host 127.0.0.1 --port 8080
```
...
| `HUB_<SERVICE>_URL` | `http://localhost:<port>` | Backend base URL, e.g. `HUB_CASAOCR_URL` |
