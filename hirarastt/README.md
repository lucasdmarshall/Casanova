...
A Hirara tool. Agents call it; developers call it — over HTTP or MCP.
...
| Module | Role |
|---|---|
| [`config.py`](src/hirarastt/config.py) | Env-driven model, device, limits (`TN_*`) |
| [`engine.py`](src/hirarastt/engine.py) | Whisper backends behind one protocol |
| [`tools.py`](src/hirarastt/tools.py) | Shared layer both front ends call |
| [`service.py`](src/hirarastt/service.py) | FastAPI HTTP |
| [`mcp_server.py`](src/hirarastt/mcp_server.py) | MCP stdio / HTTP |
...
```json
{
  "mcpServers": {
    "hirarastt": {
      "command": "python",
      "args": ["-m", "hirarastt.mcp_server"],
      "cwd": "/path/to/Hirara/hirarastt",
      "env": {
        "TN_MODEL": "base",
        "TN_DEVICE": "cpu"
      }
