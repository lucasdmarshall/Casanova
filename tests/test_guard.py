"""Tests for the SSRF perimeter.

These are the tests that matter most in this project: every case here is a
real technique someone will point at a public fetch endpoint.
"""
...
def test_rejects_v6_wrapped_internal_addresses(address):
    """A private v4 address hidden inside a v6 format is still private."""
    with pytest.raises(BlockedURL):
...
def test_rejects_unresolvable_host():
    ...
    with pytest.raises(BlockedURL, match="cannot resolve"):
        resolve_target("https://nope.invalid/", resolver=_boom)
...
def test_allows_public_address_and_pins_the_ip():
    ...
    # Default port must not appear in the Host header.
    assert target.host_header == "example.com"


def test_non_default_port_appears_in_host_header():
    target = resolve_target("http://example.com:8080/", resolver=PUBLIC)
...
def test_check_ip_accepts_public_and_rejects_private():
    ...
    assert check_ip("2606:2800:220:1::") is None
